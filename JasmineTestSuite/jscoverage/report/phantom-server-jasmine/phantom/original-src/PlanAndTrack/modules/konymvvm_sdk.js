...
function getAnimationObject(flag) {
    var animDefinition;
    if (flag == "OFF") {
        animDefinition = {
            0: {
...
kony.sdk.mvvm.AppGenUtils = Class({
    $statics: {
        isCompositeKeysPresent: function(data) {
            if (data instanceof kony.sdk.mvvm.Data) {
                data = data.getData()
            }
            if (data && data.length) {
                var firstRecord = data[0];
                var totalKeys = Object.keys(firstRecord).length;
                if (totalKeys > 1) {
                    return true
                }
            }
        ...
        format: {
            dataToCommaSeperatedCompositePrimaryKeyData: function(data, displayCompositePrimaryKey) {
                if (data instanceof kony.sdk.mvvm.Data) {
                    data = data.getData()
                }
                if (data && data.length) {
                    var primaryKeys = [];
                    var firstRecord = data[0];
                    if (!displayCompositePrimaryKey) {
                        kony.sdk.mvvm.log.info("Display field is not specified for formatting data to comma seperated compostie primary key data");
                        return data
                    }
                    if (data[0].hasOwnProperty(displayCompositePrimaryKey)) {
                        kony.sdk.mvvm.log.info(displayCompositePrimaryKey + " key is present in the record data.Hence not modifying the data");
                        return data
                    }
                    for (each in firstRecord["primaryKeyValueMap"]) primaryKeys.push(each);
                    for (var recordIndex = 0; recordIndex < data.length; recordIndex++) {
                        var commaSeperatedString = "";
                        for (var primaryKeyIndex = 0; primaryKeyIndex < primaryKeys.length; primaryKeyIndex++) {
                            commaSeperatedString += data[recordIndex]["primaryKeyValueMap"][primaryKeys[primaryKeyIndex]];
                            if (primaryKeyIndex != primaryKeys.length - 1) commaSeperatedString += ", "
                        }
                        data[recordIndex][displayCompositePrimaryKey] = commaSeperatedString
                    }
                }
                return data
            }
        ...
        HamMenu: {
            Hide: function(konyFormInstance) {
                konyFormInstance.FlexContainerMain.animate(getAnimationObject("ON"), animConfig(), {
                    animationEnd: hideEndCallBack
                });

                function hideEndCallBack() {
                    konyFormInstance.FlexContainerHamMenu.left = "-80%";
                    var hamMenu = konyFormInstance.FlexContainerHamMenu;
                    konyFormInstance.remove(hamMenu);
                    HamMenu.add(hamMenu);
                    konyFormInstance.FlexContainerMain.forceLayout()
                }
            },
            Show: function(konyFormInstance) {
                var hamMenuContainer = HamMenu.FlexContainerHamMenu;
                if (!konyFormInstance["FlexContainerHamMenu"]) {
                    HamMenu.remove(hamMenuContainer);
                    konyFormInstance.add(hamMenuContainer)
                }
                konyFormInstance.FlexContainerHamMenu.left = 0;
                konyFormInstance.FlexContainerMain.animate(getAnimationObject("OFF"), animConfig(), {
                    animationEnd: showEndCallBack
                });
        ...
        HideShowHamMenu: function(konyFormInstance) {
            if (konyFormInstance.FlexContainerMain.left !== 0) {
                this.HamMenu.Hide(konyFormInstance)
            } else {
                this.HamMenu.Show(konyFormInstance)
            }
        },
        OnHamMenuItemClick: function(konyFormInstance) {
            if (kony.application.getCurrentForm().id === konyFormInstance.id) {
                this.HamMenu.Hide(konyFormInstance)
            }
            var formController = kony.sdk.mvvm.KonyApplicationContext.getAppInstance().getFormController(konyFormInstance.id);
            formController.loadDataAndShowForm(null)
        },
        onHideForm: function(konyFormInstance) {
            if (konyFormInstance.FlexContainerMain.left !== 0) {
                konyFormInstance.FlexContainerHamMenu.left = "-80%";
                konyFormInstance.FlexContainerMain.left = 0;
                var hamMenu = konyFormInstance.FlexContainerHamMenu;
                konyFormInstance.remove(hamMenu);
                HamMenu.add(hamMenu);
                konyFormInstance.FlexContainerMain.forceLayout()
        ...
        BusinessLogicGen: {
            GoBackToForm: function(formObj, doReload, reloadForCurrentFormsNavData) {
                var instance = kony.sdk.mvvm.KonyApplicationContext.getAppInstance();
                var controller = instance.getFormController(formObj.id);
                if (doReload) {
                    var contextData = null;
                    if (reloadForCurrentFormsNavData) {
                        contextData = instance.getFormController(kony.application.getCurrentForm().id).getContextData()
                    }
                    controller.loadDataAndShowForm(contextData)
                } else {
                    controller.showForm()
                }
            },
            DeleteCurrentRecord: function(reloadFormObj, useCurrentFormsNavData) {
                var self = this;
                var instance = kony.sdk.mvvm.KonyApplicationContext.getAppInstance();
                var currentFormId = kony.application.getCurrentForm().id;
                var currentControllerExt = instance.getFormController(currentFormId).getControllerExtensionObject();
                currentControllerExt.$class.$superp.deleteData.call(currentControllerExt, deleteSuccess, deleteError);

                function deleteSuccess(res) {
                    kony.sdk.mvvm.log.info("success deleting record ", res);
                    var doReload = true;
                    self.GoBackToForm(reloadFormObj, doReload, useCurrentFormsNavData)
                }
            ...
            SaveCurrentRecord: function(reloadFormObj, useCurrentFormsNavData) {
                var self = this;
                var instance = kony.sdk.mvvm.KonyApplicationContext.getAppInstance();
                var currentFormId = kony.application.getCurrentForm().id;
                var currentControllerExt = instance.getFormController(currentFormId).getControllerExtensionObject();
                currentControllerExt.$class.$superp.saveData.call(currentControllerExt, saveSuccess, saveError);
            ...
            NavigateToAddForm: function(addFormId) {
                var navigationObject = new kony.sdk.mvvm.NavigationObject;
                navigationObject.setDataModel(null, kony.sdk.mvvm.OperationType.ADD, "form");
                var addFormController = kony.sdk.mvvm.KonyApplicationContext.getAppInstance().getFormController(addFormId);
                addFormController.loadDataAndShowForm(navigationObject)
            },
            NavigateToDetails: function(detailFormId) {
                var currentFormId = kony.application.getCurrentForm().id;
                var instance = kony.sdk.mvvm.KonyApplicationContext.getAppInstance();
                var selRecord = instance.getFormController(currentFormId).getFormModel().getViewAttributeByProperty("ListSegment", "selectedItems")[0];
                var detailController = instance.getFormController(detailFormId);
                var data = new kony.sdk.mvvm.DataModel;
                data.setPrimaryKeyValueMap(selRecord.primaryKeyValueMap);
                var navigationObject = new kony.sdk.mvvm.NavigationObject;
                navigationObject.setDataModel(data, kony.sdk.mvvm.OperationType.FILTER_BY_PRIMARY_KEY, "form");
                detailController.loadDataAndShowForm(navigationObject)
            },
            NavigateToEdit: function(editFormId) {
                var currentFormId = kony.application.getCurrentForm().id;
                var instance = kony.sdk.mvvm.KonyApplicationContext.getAppInstance();
                var editController = instance.getFormController(editFormId);
                var navigationObject = instance.getFormController(currentFormId).getContextData();
                editController.loadDataAndShowForm(navigationObject)
            }
        }
    }
});
...
kony.sdk.mvvm = kony.sdk.mvvm || {};
kony.sdk.mvvm.v2 = kony.sdk.mvvm.v2 || {};
kony.sdk.mvvm.AppFactoryMFAPP = Class(kony.sdk.mvvm.AppFactoryInterface, {
    constructor: function() {},
...
kony.sdk.mvvm.DataModelAFN = Class({
    constructor: function() {
        var masterEntityName;
        var primaryFieldValue;
        var primaryDisplayFieldValue;
        var entityDataMap = {};
        this.getMasterEntityName = function() {
            return masterEntityName
        };
        this.setMasterEntityName = function(masterEntity) {
            masterEntityName = masterEntity
        };
        ...
        this.getPrimaryFieldValue = function() {
            return primaryFieldValue
        };
        this.setPrimaryFieldValue = function(primaryFieldVal) {
            primaryFieldValue = primaryFieldVal
        };
        this.getPrimaryDisplayFieldValue = function() {
            return primaryDisplayFieldValue
        };
        this.setPrimaryDisplayFieldValue = function(primaryDisplayFieldVal) {
            primaryDisplayFieldValue = primaryDisplayFieldVal
        }
    }
});
        ...
        try {
            ...
            switch (this.getWidgetType()) {
                ...
                case "link":
                    ...
                    break;
                case "image":
                    konyWidget.rawBytes = data.getData();
                    break;
                case "calendar":
        ...
        try {
            ...
            switch (this.getWidgetType()) {
                ...
                case "link":
                    ...
                    break;
                case "image":
                    data.setData(konyWidget.rawBytes);
                    break;
                case "calendar":
    ...
    getColumnsForEntity: function(entityName) {
        ...
        for (var widget in widgets) {
            if (widgets[widget].getWidgetEntity() === entityName) {
                if (typeof widgets[widget].getField() === "object") {
                    for (var subWidget in widgets[widget].getField()) {
                        if (widgets[widget].getField()) {
                            columns.push(widgets[widget].getSubField(subWidget).field)
                        }
                    }
                } else {
                    if (widgets[widget].getField()) {
                        columns.push(widgets[widget].getField())
                    }
                }
            }
        }
...
kony.sdk.mvvm.SegmentWidgetConfig = Class(kony.sdk.mvvm.widgetConfig, {
    ...
    getWidgetDataMap: function() {
        ...
        for (var segmentKey in segmentMap) {
            ...
            if (aliasName && aliasName.toString().trim() !== "") {
                widgetDataMap[segmentKey] = aliasName
            } else if (fieldName && fieldName.toString().trim() !== "") {
                widgetDataMap[segmentKey] = fieldName
            } else {
                widgetDataMap[segmentKey] = segmentKey
            }
        }
...
kony.sdk.mvvm.KonyApplicationContext = Class(function() {
    ...
    return {
        $statics: {
            ...
            logout: function(logoutSucCallback, logoutErrCallback) {
                getApplicationContextImpl().logout(success, logoutErrCallback);
            ...
            init: function(params) {
                if (!params) params = {};
                if (getApplicationContextImpl() == undefined) {
                    var baseFactoryObjs = {};
                    baseFactoryObjs["AFN"] = kony.sdk.mvvm.AppFactoryAFN;
                    baseFactoryObjs["MF"] = kony.sdk.mvvm.AppFactoryMF;
                    baseFactoryObjs["MFAPP"] = kony.sdk.mvvm.AppFactoryMFAPP;
                    if (!params.hasOwnProperty("APPTYPE")) params.APPTYPE = "MFAPP";
                    setAppContextInstance(new kony.sdk.mvvm.KonyApplicationContext(new baseFactoryObjs[params.APPTYPE]))
                }
            }
        },
...
kony.sdk.mvvm.AuthManager = Class(kony.sdk.mvvm.BaseAuthenticationServiceManager, {
    constructor: function() {
        var appFactoryInstance = kony.sdk.mvvm.KonyApplicationContext.getFactorySharedInstance();
        this.authenticationManager = undefined;
        if (kony.sdk.mvvm.isNetworkAvailabile()) {
            this.authenticationManager = appFactoryInstance.createAuthenticationServiceManagerObject("kony.sdk.mvvm.OnlineAuthenticationServiceManager")
        } else {
            ...
            else this.authenticationManager = appFactoryInstance.createAuthenticationServiceManagerObject("kony.sdk.mvvm.OfflineAuthenticationServiceManager");
        }
    },
    ...
    saveUserDetails: function() {
        var offlineAuth = "kony.sdk.mvvm.OfflineAuthenticationServiceManager";
        if (kony.sdk.mvvm.KonyApplicationContext.getAppInstance().getApplicationProperties() && kony.sdk.mvvm.KonyApplicationContext.getAppInstance().getApplicationProperties().getApplicationPropertiesByKey("OfflineAuthenticationManager")) offlineAuth = kony.sdk.mvvm.KonyApplicationContext.getAppInstance().getApplicationProperties().getApplicationPropertiesByKey("OfflineAuthenticationManager");
        kony.sdk.mvvm.KonyApplicationContext.getFactorySharedInstance().createAuthenticationServiceManagerObject(offlineAuth).saveUserDetails()
    },
...
kony.sdk.mvvm.OfflineAuthenticationServiceManager = Class(kony.sdk.mvvm.BaseAuthenticationServiceManager, {
    constructor: function() {},
    authenticate: function(params, successCallback, errorCallback) {
        kony.sdk.mvvm.log.info("Launching app in offline mode");
        try {
            var appFactoryInstance = kony.sdk.mvvm.KonyApplicationContext.getFactorySharedInstance();
            var username = params["username"];
            ...
            var connectToUpgradedVersion = params["connectToUpgradedVersion"];
            ...
            if (connectToUpgradedVersion && (typeof connectToUpgradedVersion === "string" && connectToUpgradedVersion == "true" || typeof connectToUpgradedVersion === "boolean" && connectToUpgradedVersion == true)) this.connectToUpgradedVersion = "true";
            else this.connectToUpgradedVersion = "false";
            var appLoginDetails = {
                username: username,
                password: password,
                tenant: tenant,
            ...
            var credStore = this.getSavedUserDetails();
            if (credStore !== null && credStore !== undefined) {
                var key = credStore[kony.sdk.mvvm.credStoreSecretKey];
                var storedHostName = credStore[kony.sdk.mvvm.credStoreHostName];
                var storedUsername = credStore[kony.sdk.mvvm.credStoreUsername];
                var storedPassword = credStore[kony.sdk.mvvm.credStorePassword];
                var storedTenant = credStore[kony.sdk.mvvm.credStoreTenant];
                if (kony.sdk.mvvm.Utils.matchIgnoreCase(storedUsername, username) && kony.sdk.mvvm.Utils.matchIgnoreCase(storedPassword, password) && kony.sdk.mvvm.Utils.matchIgnoreCase(storedHostName, hostName) && kony.sdk.mvvm.Utils.matchIgnoreCase(storedTenant, tenant)) {
                    kony.sdk.mvvm.KonyApplicationContext.setAuthProfile(credStore[kony.sdk.mvvm.credStoreAuthProfile]);
                    kony.sdk.mvvm.KonyApplicationContext.getAppInstance().setOnlineStatus(false);
                    successCallback()
                } else {
                    var exception = appFactoryInstance.createExceptionObject(kony.sdk.mvvm.ExceptionCode.CD_CREDENTIAL_MISMATCH_WITH_CREDSTORE, kony.sdk.mvvm.ExceptionCode.MSG_CREDENTIAL_MISMATCH_WITH_CREDSTORE);
                    errorCallback(exception)
                }
            } else {
                kony.sdk.mvvm.KonyApplicationContext.dismissLoadingScreen();
                var exception = appFactoryInstance.createExceptionObject(kony.sdk.mvvm.ExceptionCode.CD_CREDSTORE_NOT_FOUND, kony.sdk.mvvm.ExceptionCode.MSG_CREDSTORE_NOT_FOUND);
                errorCallback(exception)
            }
        } catch (e) {
            kony.sdk.mvvm.KonyApplicationContext.dismissLoadingScreen();
    ...
    saveUserDetails: function() {
        ...
        var username = credentialObj["username"];
        var password = credentialObj["password"];
        var tenant = credentialObj["tenant"];
        var hostName = credentialObj["hostName"];
        var connectToUpgradedVersion = credentialObj["connectToUpgradedVersion"];
        var key = kony.sdk.mvvm.generateAndSaveKey("AFN");
        var encryptedUserName = kony.sdk.mvvm.encryptData(key, username);
        var encryptedPassword = kony.sdk.mvvm.encryptData(key, password);
        var storedDB = "";
        ...
        credentials_store[kony.sdk.mvvm.credStorePassword] = encryptedPassword;
        credentials_store[kony.sdk.mvvm.credStoreTenant] = tenant;
        credentials_store[kony.sdk.mvvm.credStoreHostName] = hostName;
        credentials_store[kony.sdk.mvvm.credStoreSecretKey] = key;
        credentials_store[kony.sdk.mvvm.credStoreDb] = storedDB;
        credentials_store[kony.sdk.mvvm.credStoreAuthProfile] = kony.sdk.mvvm.KonyApplicationContext.getAuthProfile();
        credentials_store[kony.sdk.mvvm.credStoreSyncCount] = syncCount;
        credentials_store[kony.sdk.mvvm.credStoreConnectToUpgradedVersion] = connectToUpgradedVersion;
    ...
    getSavedUserDetails: function() {
        ...
        if (credentialObj) {
            var key = kony.sdk.mvvm.generateAndSaveKey("AFN");
            credentialObj[kony.sdk.mvvm.credStoreUsername] = kony.sdk.mvvm.decryptData(key, credentialObj[kony.sdk.mvvm.credStoreUsername]);
            credentialObj[kony.sdk.mvvm.credStorePassword] = kony.sdk.mvvm.decryptData(key, credentialObj[kony.sdk.mvvm.credStorePassword])
        }
...
kony.sdk.mvvm.OnlineAuthenticationServiceManager = Class(kony.sdk.mvvm.BaseAuthenticationServiceManager, {
    constructor: function() {},
    authenticate: function(params, successCallback, errorCallback) {
        var scopeObj = this;
        var appFactoryInstance = kony.sdk.mvvm.KonyApplicationContext.getFactorySharedInstance();
        ...
        var hostName = params["hostName"];
        var connectToUpgradedVersion = params["connectToUpgradedVersion"];
        var checkFriendlyName = params["checkFriendlyName"];
        if (typeof username === "string") {
            this.username = username.trim()
        } else this.username = "";
        if (typeof password === "string") {
        ...
        if (checkFriendlyName == true) {
            kony.sdk.mvvm.util.getTenantInstanceID(this.tenant, this.hostName, friendlyNameServiceCallback);

            function friendlyNameServiceCallback(instanceDetails) {
                kony.sdk.mvvm.log.info("Instance details", instanceDetails);
                if (instanceDetails && instanceDetails["error"] === false) {
                    scopeObj.hostName = kony.sdk.mvvm.Utils.replaceFirstOccurence(scopeObj.tenant, instanceDetails[kony.sdk.mvvm.instanceId], scopeObj.hostName);
                    scopeObj.tenant = instanceDetails[kony.sdk.mvvm.instanceId]
                }
                authenticateService()
            }
        } else authenticateService();

        function authenticateService() {
            var appLoginDetails = {
                username: scopeObj.username,
                password: scopeObj.password,
                tenant: scopeObj.tenant,
                hostName: scopeObj.hostName,
                connectToUpgradedVersion: connectToUpgradedVersion
            };
            var dataStoreObj = kony.sdk.mvvm.KonyApplicationContext.getFactorySharedInstance().createDataStoreObject();
            dataStoreObj.storeData("UserCredentials", appLoginDetails, true);
            kony.sdk.mvvm.KonyApplicationContext.getAppInstance().setUserCredentialObj(dataStoreObj);
            if (scopeObj.hostName !== undefined && scopeObj.hostName !== null && scopeObj.hostName !== "") {
                var host;
                host = scopeObj.hostName;
                scopeObj.hostName = scopeObj.hostName.toLowerCase();
                if (scopeObj.hostName.indexOf("http://") === -1 && scopeObj.hostName.indexOf("https://") === -1) {
                    host = "https://" + host
                }
                kony.sdk.mvvm.initializeHostSettings(host);
                kony.sdk.mvvm.log.info("Host -- > " + host)
            }
            kony.sdk.mvvm.authenticateService(scopeObj.username, scopeObj.password, scopeObj.tenant, authSuccess, authError, scopeObj.connectToUpgradedVersion)
        }

        function authSuccess(result) {
            function isAFNVersionCompatible(version) {
                if (kony.sdk.mvvm.minimumSupportedVersion !== undefined && version !== undefined && version !== "*") {
                    var afnVersion = version.split(".");
                    var minSupported = kony.sdk.mvvm.minimumSupportedVersion.split(".");
                    for (var i = 0; i < minSupported.length; i++) {
                        if (afnVersion.length == i) {
                            return false
                        } else {
                            if (afnVersion[i] < minSupported[i]) {
                                return false
                            } else if (afnVersion[i] > minSupported[i]) {
                                return true
                            }
                        }
                    }
            ...
            if (version !== "*" && !isAFNVersionCompatible(version)) {
                var exception = appFactoryInstance.createExceptionObject(kony.sdk.mvvm.ExceptionCode.CD_VERSION_UNSUPPORTED_ERROR, kony.sdk.mvvm.ExceptionCode.MSG_VERSION_UNSUPPORTED_ERROR.replace("$VERSION", kony.sdk.mvvm.minimumSupportedVersion));
                kony.sdk.mvvm.KonyApplicationContext.dismissLoadingScreen();
                errorCallback(exception)
            } else {
                try {
                    kony.sdk.mvvm.log.info("Authentication Success in online mode");
                    kony.sdk.mvvm.syncSessionToken = result.token;
                    kony.sdk.mvvm.syncTenant = result.tenant;
                    if (result !== undefined && result !== null) {
                        if (result["isTenantSyncEnabled"] !== undefined && result["isTenantSyncEnabled"] !== null && result["isTenantSyncEnabled"] === true) {
                            isOnline = false
                        } else {
                            isOnline = true
                        }
                    ...
                    kony.sdk.mvvm.KonyApplicationContext.dismissLoadingScreen();
                    kony.sdk.mvvm.KonyApplicationContext.getAppInstance().setOnlineStatus(isOnline);
                    kony.sdk.mvvm.KonyApplicationContext.getAppInstance().setSessionToken(result);
                    successCallback()
                } catch (err) {
                    errorCallback(err)
                }
...
kony.sdk.mvvm.AuthManagerMF = Class(kony.sdk.mvvm.BaseAuthenticationServiceManager, {
    constructor: function() {
        ...
        this.authenticationManager = undefined;
        if (kony.sdk.mvvm.isNetworkAvailabile()) {
            this.authenticationManager = appFactoryInstance.createAuthenticationServiceManagerObject("kony.sdk.mvvm.OnlineAuthenticationServiceManagerMF")
        } else {
            var userDefinedAuthenticationManager = undefined;
            if (kony.sdk.mvvm.KonyApplicationContext.getAppInstance().getApplicationProperties()) userDefinedAuthenticationManager = kony.sdk.mvvm.KonyApplicationContext.getAppInstance().getApplicationProperties().getApplicationPropertiesByKey("OfflineAuthenticationManager");
            if (userDefinedAuthenticationManager) this.authenticationManager = appFactoryInstance.createAuthenticationServiceManagerObject(userDefinedAuthenticationManager);
            else this.authenticationManager = appFactoryInstance.createAuthenticationServiceManagerObject("kony.sdk.mvvm.OfflineAuthenticationServiceManagerMF")
        }
    },
    authenticate: function(params, successCallback, errorCallback) {
        this.authenticationManager.authenticate(params, successCallback, errorCallback)
    ...
    saveUserDetails: function() {
        var offlineAuth = "kony.sdk.mvvm.OfflineAuthenticationServiceManagerMF";
        if (kony.sdk.mvvm.KonyApplicationContext.getAppInstance().getApplicationProperties() && kony.sdk.mvvm.KonyApplicationContext.getAppInstance().getApplicationProperties().getApplicationPropertiesByKey("OfflineAuthenticationManager")) offlineAuth = kony.sdk.mvvm.KonyApplicationContext.getAppInstance().getApplicationProperties().getApplicationPropertiesByKey("OfflineAuthenticationManager");
        kony.sdk.mvvm.KonyApplicationContext.getFactorySharedInstance().createAuthenticationServiceManagerObject(offlineAuth).saveUserDetails()
    },
    getSavedUserDetails: function() {
...
kony.sdk.mvvm.OfflineAuthenticationServiceManagerMF = Class(kony.sdk.mvvm.BaseAuthenticationServiceManager, {
    constructor: function() {},
    authenticate: function(params, successCallback, errorCallback) {
        kony.sdk.mvvm.log.info("Launching app in offline mode");
        try {
...
kony.sdk.mvvm.OnlineAuthenticationServiceManagerMF = Class(kony.sdk.mvvm.BaseAuthenticationServiceManager, {
    constructor: function() {},
    authenticate: function(params, successCallback, errorCallback) {
        var appFactoryInstance = kony.sdk.mvvm.KonyApplicationContext.getFactorySharedInstance();
        var authParams = params["authParams"];
    ...
    execute: function(params, success, error) {
        ...
        this.authenticate(params, authSuccess, authError);
    ...
    constructor: function(controllerObj) {
        ...
        this.getRecordsDataMap = function(isDelete) {
            try {
                ...
                function setDataFromContext(containerName, entityName, recordObject) {
                ...
                function setDataFromWidgets(widgets, modelObj, recordObject) {
                    ...
                    for (var widget in widgets) {
                        ...
                        var fieldName = widgets[widget].getField();
                ...
                function prepareRecordsDataMap(entityDef) {
                    ...
                    for (var containerName in entityDef) {
                        ...
                        setDataFromContext(containerName, entityName, recordObject);
...
kony.sdk.mvvm.BaseFormModel = Class({
    constructor: function(controller) {
        ...
        this.notifyObservers = function(formModelPropertyName) {
            try {
                var data = formModelProperties[formModelPropertyName];
                if (!(data instanceof kony.sdk.mvvm.Data)) {
                    data = controller.getApplicationContext().getFactorySharedInstance().createDataObject(data)
                }
                ...
                throw controller.getApplicationContext().getFactorySharedInstance().createExceptionObject(kony.sdk.mvvm.ExceptionCode.CD_ERROR_NOTIFYING_OBSERVERS, kony.sdk.mvvm.ExceptionCode.MSG_ERROR_NOTIFYING_OBSERVERS, err)
            }
        };
        this.getWidgetData = function(formModelPropertyName) {
            try {
                var viewdata = this.getView().getDataFromWidget(formModelPropertyName);
                formModelProperties[formModelPropertyName] = viewdata.clone();
                return formModelProperties[formModelPropertyName]
            } catch (err) {
                kony.sdk.mvvm.log.error("error in form model get property data :" + err);
                throw controller.getApplicationContext().getFactorySharedInstance().createExceptionObject(kony.sdk.mvvm.ExceptionCode.CD_ERROR_GET_WIDGET_DATA_FORMMODEL, kony.sdk.mvvm.ExceptionCode.MSG_ERROR_GET_WIDGET_DATA_FORMMODEL, err)
            }
        };
        this.setWidgetData = function(formModelPropertyName, data, doNotNotify) {
            try {
                if (!(data instanceof kony.sdk.mvvm.Data)) {
                    data = controller.getApplicationContext().getFactorySharedInstance().createDataObject(data)
                }
                ...
                throw controller.getApplicationContext().getFactorySharedInstance().createExceptionObject(kony.sdk.mvvm.ExceptionCode.CD_ERROR_SET_WIDGET_DATA_FORMMODEL, kony.sdk.mvvm.ExceptionCode.MSG_ERROR_SET_WIDGET_DATA_FORMMODEL, err)
            }
        };
        this.isPropertyValueChanged = function(formModelPropertyName) {
            try {
                return this.getView().isWidgetValueChanged(formModelPropertyName, formModelProperties[formModelPropertyName])
            } catch (err) {
                kony.sdk.mvvm.log.error("error in form model property value change :" + err);
                throw controller.getApplicationContext().getFactorySharedInstance().createExceptionObject(kony.sdk.mvvm.ExceptionCode.CD_ERROR_FORMMODEL_PROPERTY_VALUE_CHANGED, kony.sdk.mvvm.ExceptionCode.MSG_ERROR_FORMMODEL_PROPERTY_VALUE_CHANGED, err)
            }
        };
        this.clear = function() {
            ...
            try {
                ...
                kony.sdk.mvvm.log.error("error in clear view observers.." + err);
                throw controller.getApplicationContext().getFactorySharedInstance().createExceptionObject(kony.sdk.mvvm.ExceptionCode.CD_ERROR_FORMMODEL_CLEAR, kony.sdk.mvvm.ExceptionCode.MSG_ERROR_FORMMODEL_CLEAR, err)
            }
        };
        this.destroy = function() {
            try {
                ...
                throw controller.getApplicationContext().getFactorySharedInstance().createExceptionObject(kony.sdk.mvvm.ExceptionCode.CD_ERROR_FORMMODEL_DESTROY, kony.sdk.mvvm.ExceptionCode.MSG_ERROR_FORMMODEL_DESTROY, err)
            }
        };
        this.update = function(widgetArray) {
            try {
                var data;
                if (!widgetArray) {
                ...
                for (var index in widgetArray) {
                    data = formModelProperties[widgetArray[index]];
                    for (var key in this.getObserverViews()) {
                ...
                throw controller.getApplicationContext().getFactorySharedInstance().createExceptionObject(kony.sdk.mvvm.ExceptionCode.CD_ERROR_FORMMODEL_UPDATE, kony.sdk.mvvm.ExceptionCode.MSG_ERROR_FORMMODEL_UPDATE, err)
            }
        };
...
kony.sdk.mvvm.BaseModel = Class({
    constructor: function(applicationContext, entityMetaData, configOptions) {
        ...
        this.getOperationOptions = function(options) {
    ...
    create: function(options, onSuccess, onError) {
        try {
            var scopeObj = this;
            var operationOptions = scopeObj.getOperationOptions(options);
            if (false === this.validate(operationOptions["dataObject"], kony.sdk.mvvm.v2.Model.ValidationType.CREATE)) {
            ...
            scopeObj.getObjectService().create(operationOptions, success, error)
        } catch (e) {
    ...
    update: function(options, onSuccess, onError) {
        ...
        try {
            var scopeObj = this;
            var operationOptions = scopeObj.getOperationOptions(options);
            if (false === this.validate(operationOptions["dataObject"], kony.sdk.mvvm.v2.Model.ValidationType.UPDATE)) {
            ...
            if (this.getAccessType() === "offline") scopeObj.getObjectService().update(operationOptions, success, error);
            else scopeObj.getObjectService().partialUpdate(operationOptions, success, error)
        } catch (err) {
    ...
    completeUpdate: function(options, onSuccess, onError) {
        ...
        try {
            ...
            scopeObj.getObjectService().update(operationOptions, success, error)
        } catch (err) {
            var exception = scopeObj.getApplicationContext().getFactorySharedInstance().createExceptionObject(kony.sdk.mvvm.ExceptionCode.CD_ERROR_UPDATE, kony.sdk.mvvm.ExceptionCode.MSG_ERROR_UPDATE, err);
            onError(exception)
        }
    ...
    remove: function(options, onSuccess, onError) {
        try {
            ...
            var operationOptions = scopeObj.getOperationOptions(options);
            scopeObj.getObjectService().deleteRecord(operationOptions, success, error)
        } catch (err) {
    ...
    fetchResponse: function(options, onSuccess, onError) {
        ...
        try {
            var self = this;
            var operationOptions = self.getOperationOptions(options);
            self.getObjectService().fetch(operationOptions, success, error)
        } catch (e) {
            var exception;
...
kony.sdk.mvvm.persistent.GroupWidgetsContextCommon = Class({
    constructor: function(configObject, contextDataObj) {
        ...
        this.generateWidgetContext = function(widgetId, widgetConfig) {
            ...
            widgetContext["options"] = config.getObjectServiceOptions();
            if (widgetConfig["child"]) {
                childWidgets = Object.keys(widgetConfig["child"])
            }
            widgetContext["child"] = childWidgets || [];
            var entityName, query, queryParams, columns;
            if (widgetId === "form") {
                entityName = config.getEntity();
                query = config.getQuery();
                queryParams = config.getQueryParams() || {};
                columns = config.getAdditionalFields()
            } else {
                entityName = widgetConfig["config"].getWidgetEntity();
...
kony.sdk.mvvm.persistent.GroupWidgetsContextOffline = Class({
    ...
    fetchDataForGroupWidget: function(widgetId, successCallback, errorCallback) {
        ...
        var options = widgetContext["options"];
        ...
        if (query && query.toLowerCase().indexOf("select") > -1) {
            if (filter) {
                ORMController.fetchByNativeQuery(entityName, filter, serviceName, options, succCallback, errCallback)
            }
        } else if (dataModel && dataModel.getPrimaryKeyValueMap() && operationType === kony.sdk.mvvm.OperationType.FILTER_BY_PRIMARY_KEY) {
            ORMController.fetchRecordByPrimaryKey(entityName, columnsdef, filter, dataModel.getPrimaryKeyValueMap(), serviceName, options, succCallback, errCallback)
        } else if (!operationType || operationType === kony.sdk.mvvm.OperationType.NO_FILTER) {
            ORMController.fetchByColumns(entityName, columnsdef, filter, serviceName, options, succCallback, errCallback)
        } else {
    ...
    updateChildWidgetsFilter: function(widgetContext, resultRow) {
        ...
        for (var indx in childGroupWidgets) {
            ...
            var updatedQuery = ORMController.updateChildFilter(entityName, serviceName, options, childEntityName, query, resultRow);
...
kony.sdk.mvvm.persistent.GroupWidgetsContextOnline = Class({
    ...
    updateChildResponse: function(widgetContext, response, result) {
        ...
        for (var indx in childGroupWidgets) {
            ...
            var childEntityName = childWidgetContext["entityName"];
            var query = childWidgetContext["query"];
            var updatedQuery = ORMController.updateChildFilter(entityName, serviceName, options, childEntityName, query, response);
            childWidgetContext["query"] = updatedQuery;
...
kony.sdk.mvvm.persistent.ORMControllerMFAPP = Class(kony.sdk.mvvm.persistent.ORMControllerInterface, {
    ...
    fetchRecordByPrimaryKey: function(entityName, columnsdef, filter, primaryFieldValueMap, serviceName, options, onSuccess, onError) {
        this.getORMController().fetchRecordByPrimaryKey(entityName, columnsdef, filter, primaryFieldValueMap, serviceName, options, onSuccess, onError)
    },
    fetchByNativeQuery: function(entityName, queryObj, serviceName, options, onSuccess, onError) {
        this.getORMController().fetchByNativeQuery(entityName, queryObj, serviceName, options, onSuccess, onError)
    },
    fetchByColumns: function(entityName, columnsdef, filter, serviceName, options, onSuccess, onError, filterByKey) {
        this.getORMController().fetchByColumns(entityName, columnsdef, filter, serviceName, options, onSuccess, onError, filterByKey)
    },
    ...
    saveChildRecords: function(entityName, childRecordsList, record, response, serviceName, options, childSuccess, childError) {
        this.getORMController().saveChildRecords(entityName, childRecordsList, record, response, serviceName, options, childSuccess, childError)
    },
...
kony.sdk.mvvm.persistent.ORMControllerMFAPPSuper = Class(kony.sdk.mvvm.persistent.ORMControllerInterface, {
    ...
    saveRecord: function(record, onSuccess, onError) {
        try {
            ...
            if (operation && operation === kony.sdk.mvvm.OperationType.ADD) {
                modelObj.create(dataObject, success, error)
            } else if (primaryValuesExist(record, primaryKeyArr)) {
                modelObj.update(dataObject, success, error)
            } else {
                modelObj.create(dataObject, success, error)
            }

            function success(response) {
                scopeObj.saveChildRecords(entityName, childRecordsList, record, response, serviceName, options, childSuccess, childError);
    ...
    removeRecord: function(record, onSuccess, onError) {
        try {
            ...
            function success() {
                ...
                modelObj.removeByPrimaryKey(primaryKeyValueMap, succCallback, errCallback);
    ...
    return {
        ...
        fetchRecordByPrimaryKey: function(entityName, columnsdef, filter, primaryFieldValueMap, serviceName, options, onSuccess, onError) {
            try {
                ...
                this.fetchByColumns(entityName, columnsdef, filter, serviceName, options, onSuccess, onError)
            } catch (e) {
        ...
        fetchByNativeQuery: function(entityName, query, serviceName, options, onSuccess, onError) {
            try {
                var scopeObj = this;
                var modelObj = this.getModel(entityName, serviceName, options);
                var dataObject = new kony.sdk.dto.DataObject(entityName);
                dataObject.setOdataUrl(query);
                modelObj.fetchResponseWithRecords(dataObject, success, error);
        ...
        fetchByColumns: function(entityName, columnsdef, filter, serviceName, options, onSuccess, onError) {
            try {
                var scopeObj = this;
                var entityCtrlr = this.getModel(entityName, serviceName, options);
                var queryMap = this.getFetchQueryMap(entityName, columnsdef, serviceName, options);
                this.fetchDataMap(queryMap, filter, serviceName, options, success, onError);
        ...
        fetchDataMap: function(queryMap, filter, serviceName, options, onSuccess, onError) {
            var scopeObj = this;
            var resultSet = {};
            ...
            var entityCtrlr = scopeObj.getModel(entityName, serviceName, options);
            var dataObject = new kony.sdk.dto.DataObject(entityName);
            if (query) {
                dataObject.setOdataUrl(query)
            }
            entityCtrlr.fetchResponseWithRecords(dataObject, success, error);

            function success(response) {
                resultSet[entityName] = response;
                scopeObj.fetchJoinsDataMap(queryMap.joins, resultSet, serviceName, options, joinSuccess, error)
            }
        ...
        fetchJoinsDataMap: function(queryMapList, resultSet, serviceName, options, onSuccess, onError) {
            var scopeObj = this;
            var indx = 0;
            ...
            function fetchJoinDataMap(indx) {
                ...
                if (resultSet[queryMap.parentEntity] && resultSet[queryMap.parentEntity]["records"] && resultSet[queryMap.parentEntity]["records"].length === 1) {
                    filter = getFilterString(queryMap.relFields, resultSet[queryMap.parentEntity]["records"])
                }
                scopeObj.fetchDataMap(queryMap, filter, serviceName, options, success, onError);
...
kony.sdk.mvvm.persistent.ORMControllerMFAPPODataExpand = Class(kony.sdk.mvvm.persistent.ORMControllerMFAPPSuper, function() {
    ...
    return {
        ...
        fetchByNativeQuery: function(entityName, query, serviceName, options, onSuccess, onError) {
            this.getORMController().fetchByNativeQuery(entityName, query, serviceName, options, onSuccess, onError)
        },
        fetchByColumns: function(entityName, columnsdef, filter, serviceName, options, onSuccess, onError) {
            try {
                var scopeObj = this;
        ...
        fetchDataMap: function(queryMap, filter, serviceName, options, onSuccess, onError) {
            this.getORMController().fetchDataMap(queryMap, filter, serviceName, options, onSuccess, onError)
        },
        updateChildFilter: function(entityName, serviceName, options, childEntityName, query, parentRow) {
            return this.getORMController().updateChildFilter(entityName, serviceName, options, childEntityName, query, parentRow)
        }
        ...
        fetchJoinsDataMap: function(queryMapList, resultSet, serviceName, options, onSuccess, onError) {
            ...
            function fetchJoinDataMap(indx) {
                ...
                if (resultSet[queryMap.parentEntity] && resultSet[queryMap.parentEntity].length === 1) {
                    filter = getFilterString(queryMap.relFields, resultSet[queryMap.parentEntity])
                }
                scopeObj.fetchDataMap(queryMap, filter, serviceName, options, success, onError);
            ...
            try {
                ...
                for (var key in record) {
                    ...
                    for (var field in fields) {
                        ...
                        len = fieldNameArray.length;
                        if (!aliasName) {
                            aliasName = fieldNameArray[len - 1]
                        }
                        propData = record[key];
                        for (var i = 1; i < len; i++) {
...
kony.sdk.mvvm.SyncManagerMF = Class(kony.sdk.mvvm.AppInitServiceMangerInterface, {
    ...
    startSyncForScope: function(syncscopeSuccesscallback, syncscopeErrorcallback, isMetadata, config) {
        ...
        var batchSize = "10";
        if (!config.hasOwnProperty("batchsize")) config.batchsize = batchSize;
        ...
        function syncError() {
            kony.sdk.mvvm.onsyncerrorCallback();
            syncscopeErrorcallback()
        }
    },
