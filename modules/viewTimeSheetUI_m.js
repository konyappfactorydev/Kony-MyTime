...
processTimesheetComments = function (commentsData) {
	...
	for (var i = 0; i < commentsData.length; i++) {
		var comment = {};      
		if (commentsData[i].Employee_Id === kony.apps.coe.ess.myTime.ViewTimeSheetUI.empId) {
			...
			comment.lblApprovedDate = (commentsData[i].Added_On==="")?"":new Date().modifyByYYYYMMDDHHMMSS(commentsData[i].Added_On).toHHMMMHHmm();
			...
			comment.lblApplier = commentsData[i].name;
          	comment.lblCommentInitials = {isVisible : true, text : getNameInitials(commentsData[i].name)};;
			comment.imgUser = {isVisible : false};
			comment.lblAppliedDate = new Date().modifyByYYYYMMDDHHMMSS(commentsData[i].Added_On).toHHMMMHHmm();
