/*
 * Controller Extension class for frmTimeSheetCreateTab
 ...
 * @class frmTimeSheetCreateTabControllerExtension
 * @param {Object} controllerObj - Form Controller.
 */
kony.sdk.mvvm.frmTimeSheetCreateTabControllerExtension = Class(kony.sdk.mvvm.BaseFormControllerExtension, {
    ...
    fetchData: function() {
        try {
          ...
          kony.sdk.mvvm.KonyApplicationContext.showLoadingScreen("Loading Form");
            ...
            var extremeDates = kony.apps.coe.ess.myTime.TimesheetDatesSectionTab.getCurrentTimesheetData(this.getController().getContextData()); 
    ...
    bindData: function(data) {
        try {
            var formmodel = this.getController().getFormModel();
                ...
                //kony.apps.coe.ess.myTime.TimesheetCreate.BackendTab.contextData = hello();
                //kony.apps.coe.ess.myTime.TimesheetCreate.AddTimelineToForm();
                kony.apps.coe.ess.myTime.TimesheetCreate.WorkLeaveToggleTab.onClickOfWork();
				kony.apps.coe.ess.myTime.timesheetHome.TimesheetRowObjTab = new kony.apps.coe.ess.myTime.TimesheetRowTab();
				//kony.apps.coe.ess.myTime.timesheetHome.TimesheetDatesSectionObj = new kony.apps.coe.ess.myTime.TimesheetDatesSection(frmTimesheetHome.flxDateInDateSection, frmTimesheetHome.flxScrlDatesSection);
              	if(kony.apps.coe.ess.myTime.timesheetHome.TimesheetDatesSectionObjTab === null || kony.apps.coe.ess.myTime.timesheetHome.TimesheetDatesSectionObjTab === undefined) {
                    kony.apps.coe.ess.myTime.timesheetHome.TimesheetDatesSectionObjTab = new kony.apps.coe.ess.myTime.TimesheetDatesSectionTab(frmTimeSheetCreateTab.flxDateInDateSection, frmTimeSheetCreateTab.flxScrlDatesSection);    
                }
				kony.apps.coe.ess.myTime.timesheetHome.TimesheetDatesSectionObjTab.labelDataMap = "displayValue";
				kony.apps.coe.ess.myTime.timesheetHome.TimesheetDatesSectionObjTab.setData(data); //Setting Data
				kony.apps.coe.ess.myTime.timesheetHome.TimesheetDatesSectionObjTab.setOnSelectionCallback(kony.apps.coe.ess.myTime.timesheetHome.onDateSelectionTab); //ToDo : Add Function Here
                ...
                for(var i = 0; i < data.length; i++) {
                    if(date.compareOnlyDate(data[i].startDate) >= 0 && date.compareOnlyDate(data[i].endDate) <= 0) {
                        kony.apps.coe.ess.myTime.timesheetHome.TimesheetDatesSectionObjTab.setSelectedItem(i);
                        break;
                    }
                }
               // kony.apps.coe.ess.myTime.TimesheetCreate.Backend.contextDataTab = arr;
                //alert(kony.apps.coe.ess.myTime.TimesheetCreate.Backend.contextDataTab);
				kony.sdk.mvvm.KonyApplicationContext.dismissLoadingScreen();
...
kony.apps.coe.ess.myTime.getTimesheetIDTab = function (fromDateString, toDateString, successCall, failureCall) {
...
kony.apps.coe.ess.myTime.timesheetHome.fetchTimeEntriesTab = function (timesheetID, weekStart, weekEnd, successCall, failureCall) {
...
//         " AND TE.Date <= '"+toDate+"' " +
    ...
    kony.sync.single_select_execute(kony.sync.getDBName(), sqlQuery, null, function (res) {
		//Query Success
		kony.apps.coe.ess.myTime.timesheetHome.processTimesheetEntriesTab(res, successCall);
	}, function (err) {
		//Query Failed
		failureCall(err);
	});
 ...
 *		Success Callback. Array of Task Entries are returned/passed as input param
 */
kony.apps.coe.ess.myTime.timesheetHome.processTimesheetEntriesTab = function (res, successCall) {
	...
	var tempstartdate = kony.apps.coe.ess.myTime.timesheetHome.TimesheetDatesSectionObjTab.getSelectedItemData().startDate;
...
kony.apps.coe.ess.myTime.timesheetHome.updateStatusForAllRowsTab = function(date, data) {
    kony.apps.coe.ess.myTime.getTimesheetDataForADate(date, function(data, tsd) {
        ...
        if(tsd !== null) {
            ...
            kony.apps.coe.ess.myTime.timesheetHome.TimesheetRowObjTab.setData(data, timesheetinterval[0], timesheetinterval[1]);
            //new (kony.apps.coe.ess.myTime.TimesheetRow.Tab).setData(data, timesheetinterval[0], timesheetinterval[1]);
        } else {
            kony.apps.coe.ess.myTime.timesheetHome.TimesheetRowObjTab.setData(data, timesheetinterval[0], timesheetinterval[1]);
           //new (kony.apps.coe.ess.myTime.TimesheetRow.Tab).setData(data, timesheetinterval[0], timesheetinterval[1]);
        }
...
kony.apps.coe.ess.myTime.timesheetHome.onDateSelectionTab = function (weekData) {
    var date = null;
    if (kony.apps.coe.ess.globalVariables.timesheetLengthConfig.toLowerCase() === "monthly" &&
		parseInt(kony.apps.coe.ess.myTime.timesheetHome.TimesheetDatesSectionObjTab._selectedItem) === 0) {
	...
	var callback = function (weekData, tsd) {
        ...
        kony.apps.coe.ess.myTime.timesheetHome.TimesheetDatesSectionObjTab._data[kony.apps.coe.ess.myTime.timesheetHome.TimesheetDatesSectionObjTab._selectedItem].timesheetId = tsid;
		frmTimeSheetCreateTab.lblCurrentDateMonth.text = kony.apps.coe.ess.myTime.nToStr.fullmonth[weekData.startDate.getMonth()];
	//#ifndef windows8	
      frmTimeSheetCreateTab.lblCurrentDateYear.text = weekData.startDate.getFullYear().toFixed(0);
      //#else
      frmTimeSheetCreateTab.lblCurrentDateMonth.text +=" "+weekData.startDate.getFullYear().toFixed(0);
      //#endif
		kony.apps.coe.ess.myTime.timesheetHome.fetchTimeEntriesTab(weekData.timesheetId, weekData.startDate, weekData.endDate, function (res) {
                //if(res.tasks !== undefined && res.tasks !== null && data.tasks.length > 0)
                kony.apps.coe.ess.myTime.TimesheetCreate.BackendTab.contextData=res;
                //kony.apps.coe.ess.myTime.TimesheetCreate.AddTimelineToFormTab1(res);
			//FetchTimesheetEntries Success
			kony.apps.coe.ess.myTime.timesheetHome.TimesheetRowObjTab.setOnSelectionCallbackRight(function (data) {
			...
			if (kony.apps.coe.ess.globalVariables.timesheetLengthConfig.toLowerCase() === "monthly" &&
				parseInt(kony.apps.coe.ess.myTime.timesheetHome.TimesheetDatesSectionObjTab._selectedItem) === 0) {
				kony.apps.coe.ess.myTime.TimesheetCreate.validateSubmitButtonVisibilityTab(weekData.endDate);
                kony.apps.coe.ess.myTime.timesheetHome.updateStatusForAllRowsTab(weekData.endDate, res);
			} else {
				kony.apps.coe.ess.myTime.TimesheetCreate.validateSubmitButtonVisibilityTab(weekData.startDate);
                kony.apps.coe.ess.myTime.timesheetHome.updateStatusForAllRowsTab(weekData.startDate, res);
			}
		}, function (err) {
