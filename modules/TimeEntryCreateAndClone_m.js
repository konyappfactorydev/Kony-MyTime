...
kony.apps.coe.ess.myTime.TimesheetCreate.TimeEntry = {
    ...
    create: function(values, callback, errorCallback) {
        ...
        var successCallback = function(me, size, callback, i, res) {
            //Committed for now changes on this varaible are not reflected in cloning
            //var timeEntryRecordNo=kony.apps.coe.Reusable.TimelineCreation.TimeSheetData.length-1;
            //kony.apps.coe.Reusable.TimelineCreation.TimeSheetData[timeEntryRecordNo].data.Time_Entry_Id=(res.Id).toString();
            if (i >= size - 1) {
